# Example Script to create wind particle flow animation
...
ob = readRDS('./data/Particles.RDS')
setkey(ob, datetime_)
ob[, w_speed := sqrt(u^2 + v^2)]

# Set time frame and path
ts = data.table(date = seq(min(w$datetime_) %>% as.Date %>% as.POSIXct, max(w$datetime_) %>% as.Date %>% as.POSIXct, by = '30 mins') )
setkey(ts, date)
tmp_path = './temp/' # change path
...
# cl = 4 %>% makePSOCKcluster; registerDoParallel(cl) # 4 cores
...
foreach(i = 1:nrow(ts), .packages = c('scales', 'ggplot2', 'lubridate', 'stringr', 'data.table', 'windR') ) %dopar% {
  ...
  png(filename = ts[i, path], width = 700, height = 700, units = "px", pointsize = 9, bg = "white") # high resolution
